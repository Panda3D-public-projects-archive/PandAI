...
class World(DirectObject):
    ...
    def __init__(self):
        ...
        self.title = addTitle(
            "Pandai Tutorial: Dynamic Avoidance of Moving Obstacles")
        self.inst1 = addInstructions(0.95, "[ESC]: Quit")
        self.inst2 = addInstructions(0.90, "[Arrow Keys]: Move the blue Ralph")
        self.inst3 = addInstructions(
            0.85,
            "Try and move the blue Ralph in the path of the other Ralphs")
    ...
    def loadModels(self):
        ...
        self.environ = base.loader.loadModel("models/plane_demo1")
        ...
        for i in range(4):
            self.ralph.append(Actor("models/ralph",
                                    {"run": "models/ralph-run",
                                     "walk": "models/ralph-walk"}))
            self.ralph[i].reparentTo(base.render)
            self.ralph[i].setScale(2)

            self.positions.append(NodePath(str(i)))
            self.positions_new.append(NodePath(str(i)))
            if (i < 2):
                self.ralph[i].setPos(-61, -34 + (i * 40), 0)
            else:
                self.ralph[i].setPos(61, -34 + ((i-2) * 40), 0)

            self.positions.append(NodePath(str(i)))
            self.positions_new.append(NodePath(str(i)))

        self.positions[0].setPos(-61, -34 + ((0) * 40), 0)
        self.positions[1].setPos(-53, -34 + ((1) * 40), 0)
        self.positions[2].setPos(53, -44 + ((0) * 40), 0)
        self.positions[3].setPos(61, -24 + ((1) * 40), 0)

        self.positions_new[0].setPos(61, -44 + ((0) * 40), 0)
        self.positions_new[1].setPos(53, -44 + ((1) * 40), 0)
        self.positions_new[2].setPos(-53, -24 + ((0) * 40), 0)
        self.positions_new[3].setPos(-61, -24 + ((1) * 40), 0)
    ...
    def setAI(self):
        ...
        for i in range(4):
            self.AIchar.append(AICharacter(
                "ralph", self.ralph[i], 60, 0.05, 25 - (5 * random.random())))
    ...
    def setMove(self, type):
        if (type == 1):
            for i in range(4):
                if (i == 0):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions_new[0], "addPath")
                    self.AIbehaviors[i].addDynamicObstacle(self.ralph[2])
                if (i == 1):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions_new[1], "addPath")
                    self.AIbehaviors[i].addDynamicObstacle(self.ralph[3])
                if (i == 2):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions_new[2], "addPath")
                if (i == 3):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions_new[3], "addPath")
        ...
        if (type == 2):
            for i in range(4):
                if (i == 0):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions[0], "addPath")
                if (i == 1):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions[1], "addPath")
                if (i == 2):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions[2], "addPath")
                if (i == 3):
                    self.AIbehaviors[i].pathFindTo(
                        self.positions[3], "addPath")
    ...
    def Mover(self, task):
        ...
        if (self.keyMap["forward"] != 0):
            forwardvector = self.Targetforward.getPos(base.render) - startPos
