...
from panda3d.egg import EggPolygon, EggGroupNode, EggData
        ...
        for r in range(int(sqrt(self.nodeCount))):
            for c in range(int(sqrt(self.nodeCount))):
                ...
                if node is None:
                    ...
                    # Length
                    file.write(
                        str(node.vertex[0].getX() - node.vertex[1].getX()) + ',')
                    # Width
                    file.write(
                        str(node.vertex[0].getZ() - node.vertex[3].getZ()) + ',')
                    # Height
                    file.write('0' + ',')
                    # PosX
                    file.write(
                        str((node.vertex[0].getX() + node.vertex[1].getX())/2) + ',')
                    # PosY
                    file.write(
                        str((node.vertex[0].getZ() + node.vertex[3].getZ())/2) + ',')
                    # PosZ
                    file.write('0')
                    ...
                    for i in range(8):
                        ...
                        if nnode is None:
                            ...
                            # Length
                            file.write(
                                str(nnode.vertex[0].getX() - nnode.vertex[1].getX()) + ',')
                            # Width
                            file.write(
                                str(nnode.vertex[0].getZ() - nnode.vertex[3].getZ()) + ',')
                            # Height
                            file.write('0' + ',')
                            # PosX
                            file.write(
                                str((nnode.vertex[0].getX() + nnode.vertex[1].getX())/2) + ',')
                            # PosY
                            file.write(
                                str((nnode.vertex[0].getZ() + nnode.vertex[3].getZ())/2) + ',')
                            # PosZ
                            file.write('0')
    ...
    def analyzeVertex(self, node):
        ...
        for i in range(4):
            if not (i == self.lowestVertex):
                x1 = float(
                    int(node.vertex[self.lowestVertex].getX()*precision)/precision)
                x2 = float(int(node.vertex[i].getX()*precision)/precision)
                z1 = float(
                    int(node.vertex[self.lowestVertex].getZ()*precision)/precision)
